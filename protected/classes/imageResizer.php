...
class imageResizer
{
	...
	public function resize($wx, $hx, $gab1, $gab2) {
        ...
        $color = imagecolorallocate($this->copy, 255, 255, 255);
        imagefill($this->copy, 0, 0, $color);

		imagecopyresampled($this->copy, $this->image, $dx, $dy, $sx, $sy, $dw, $dh, $sw, $sh);
		/*if($this->data['mime']=='image/png')
		{
			imagealphablending($this->copy, false);
			$color  = imagecolorallocatealpha($this->copy, 0, 0, 0, 127);
			imagefill($this->copy, 0, 0, $color);
