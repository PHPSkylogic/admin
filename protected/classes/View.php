		...
		public function Render($includeFile, $vars = array())
		{
			...
			elseif(isset($this->registry['admin']))
			{
				...
				if(!file_exists($pathTpl))
				{
					...
					$pathTpl = MODULES.strtolower($this->registry['admin'])."/admin"."/".$this->tplDir.$includeFile;//echo $pathTpl."<br />";
					if(!file_exists($pathTpl))
					{
						$pathTpl = SUBSYSTEM.strtolower($this->registry['admin'])."/".$this->tplDir.$includeFile;//echo $pathTpl."<br />";
						if(!file_exists($pathTpl))
						{
							Log::echoLog('Could not found template \'' . $pathTpl . '\' !!!');
							return;
