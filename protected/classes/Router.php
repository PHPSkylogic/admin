			...
			elseif($url[0]=="admin")
			{
				...
				elseif(checkAuthAdmin())
				{
					...
					if(isset($url[1]))
					{
						...
						if($row2)
						{
							$param = array($row2['id'], $_SESSION['admin']['id']);
							$row = $this->db->row("SELECT mm.`permission`, m.type_moderator
												   FROM `moderators` m
												   
												   LEFT JOIN `moderators_permission` mm
												   ON mm.moderators_type_id=m.type_moderator AND mm.module_id=?
