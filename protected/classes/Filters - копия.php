...
class Filters{
    ...
    {	
		...
		foreach($filters as $row)
		{
			...
			if($param!='')
			{
				$product = $this->db->row("SELECT GROUP_CONCAT(DISTINCT p.id ORDER BY p.id ASC SEPARATOR ',') as id
										   ...
										   WHERE p.active='1' AND $sub_cat ".$param."");
			...
			$query = "  SELECT tb1.id, tb1.url, tb1.sub, tb2.name, $select_count, pp.product_id, pp.params_id, GROUP_CONCAT(DISTINCT pp.product_id ORDER BY pp.product_id ASC SEPARATOR ',') as product_id
		...
		if($params_url!='')
		{
			$current_product=explode(',', $current_product);echo count($current_product);
			$i=0;
