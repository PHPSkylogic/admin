...
class Importexel extends Model{
	...
	function export_products()
	{
		...
		$query="SELECT tb.id AS export_id, 
					   ...
					   tb.discount AS export_discount, 
					   ...
					   c2_sub.name as export_cat_name2, 
					   c_sub.id as export_catalog_id2,
				...
				LEFT JOIN product_catalog pc_sub
				ON tb.id=pc_sub.product_id AND pc_sub.catalog_id!=pc.catalog_id
				
				LEFT JOIN catalog c_sub
				ON pc_sub.catalog_id=c_sub.id
				
				LEFT JOIN ru_catalog c2_sub
				ON c_sub.id=c2_sub.catalog_id
				
				LEFT JOIN (SELECT * FROM `price` ORDER BY `sort` ASC, id DESC) as `tb_price`
				ON `tb_price`.product_id=tb.id
				
				LEFT JOIN params_product pp
				ON pp.product_id=tb.id
		...
		if(count($res)!=0)
		{
			...
			foreach($res as $row)
			{
				if($row['export_cat_name2']!='')
				{
					$row['export_cat_name']=$row['export_cat_name2'];
					$row['export_catalog_id']=$row['export_catalog_id2'];
				}
				...
				if(!isset($row[$arr['M']]))$row[$arr['M']]='';
				if(!isset($row[$arr['N']]))$row[$arr['N']]='';
...
', '', $str);
	...
	function abc()
	{
		...
		if(!isset($arr['M']))$arr['M']='';
		if(!isset($arr['N']))$arr['N']='';
		...
		if(!isset($arr['M2']))$arr['M2']='';
		if(!isset($arr['N2']))$arr['N2']='';
	...
	function import_products($data=array())
	{
		...
		foreach($res as $row)
		{
			...
			if(isset($_POST['brend_name'])&&$_POST['brend_name']!='')
			{
				...
				if(!isset($brend['id']))
				{
					$url=STRING::translit($row[$_POST['brend_name']]);
					$www="";
			...
			if(!isset($update)&&$where2!='')
			{
				...
				{
					$where.=",url='".$insert_id.'-'.STRING::translit($row[$_POST['name']])."'";
			...
			elseif(isset($update))
			{
				...
				if(isset($_POST['price'])&&$_POST['price']!='')
				{
					...
					if(isset($row[$_POST['code']]))$www="code='{$row[$_POST['code']]}', ";
					if(isset($row[$_POST['stock']]))$www.="stock='{$row[$_POST['stock']]}', ";
					$this->db->query("UPDATE `price` SET $www `price`='".$row[$_POST['price']]."', `discount`='".$row[$_POST['discount']]."' WHERE id='{$row_price['id']}'");
				}
			...
			if(isset($_POST['cat_name'])&&$_POST['cat_name']!='')
			{
				...
				if(!isset($cat['id']))
				{
				    if(!isset($row[$_POST['catalog_id']]))$row[$_POST['catalog_id']]=NULL;
					$insert_id=$this->db->insert_id("INSERT INTO catalog SET active='1', id='".$row[$_POST['catalog_id']]."'");
					foreach($this->language as $row2)
					{
			...
			if(isset($row[$_POST['params']])&&$row[$_POST['params']]!=''&&$_POST['params']!='')
			{
				...
				foreach($params as $row5)
				{
					if($row5!=''&&$row[$_POST['id']]!='')
					{
						...
						$cat=$this->db->row("SELECT params_id FROM params_product WHERE params_id=? AND product_id=?", $param);	
						if(!isset($cat['params_id']))
						{
	...
	function select_cols()
	{
		$return=array();
		$return['select']='';
		$fields=array('id', 'name', 'body', 'body_m', 'code', 'image', 'price', 'discount', 'stock', 'catalog_id', 'cat_name', 'params');
