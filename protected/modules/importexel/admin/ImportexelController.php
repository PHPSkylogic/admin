...
class ImportexelController extends BaseController{
	...
	public function indexAction()
	{
        ...
        ////XML
        if (isset($_POST['updateYandex']))
        {
            $yandexImport = new yandexImport($this->sets);
			if (move_uploaded_file($_FILES['yandex_file']['tmp_name'], 'files/xml/yandexMarket.xml'))
            {
				$vars['message'] = $yandexImport->importXml('files/xml/yandexMarket.xml', true);
			}
			else if (file_put_contents('files/xml/yandexMarket.xml', file_get_contents(trim($_POST['yandex_link']))))
            {
				$vars['message'] = $yandexImport->importXml('files/xml/yandexMarket.xml', true);
			}
	   }
