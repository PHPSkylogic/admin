...
class Slider extends Model
{
    ...
    public function add()
    {
        ...
        if(isset($_POST['name'], $_POST['url']))
        {
			...
			{
				$this->db->query("INSERT INTO `".$row['language']."_".$this->table."` SET `name`=?, `body`=?, slider_id=?", array($_POST['name'], $_POST['body'], $id));
			}
    ...
    public function save()
    {
        ...
        else
        {
            if(isset($_POST['save_id'])&&is_array($_POST['save_id']))
            {
                ...
                {
                    $url = String::translit($_POST['url'], true);

                     $param = array($_POST['active'], $url, $_POST['id']);
                    $this->db->query("UPDATE `".$this->table."` SET `active`=?, url=? WHERE id=?", $param);
        ...
        else
        {
            if(isset($_POST['id'])&&is_array($_POST['id']))
            {
                for($i=0; $i<=count($_POST['id']) - 1; $i++)
                {
					...
					foreach($this->language as $row)
					{
						$this->db->query("DELETE FROM `".$row['language']."_".$this->table."` WHERE `slider_id`=?", array($_POST['id'][$i]));	
					}
            ...
            elseif(isset($this->params['delete'])&& $this->params['delete']!='')
            {
				...
				foreach($this->language as $row)
				{
					$this->db->query("DELETE FROM `".$row['language']."_".$this->table."` WHERE `slider_id`=?", array($id));	
				}
