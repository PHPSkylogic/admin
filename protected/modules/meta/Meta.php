...
class Meta extends Model
{
	...
	public function add()
	{
		...
		if(isset($_POST['active'], $_POST['url'], $_POST['title'], $_POST['keywords'], $_POST['description'], $_POST['body'], $_POST['excerpt'])&&$_POST['url']!="")
		{
			...
			$id = $this->db->insert_id("INSERT INTO `".$this->table."` SET `url`=?, `active`=?, `type`=?", $param);
	...
	{
		$vars=array();
		$vars['product'] = Product::getObject($this->sets)->find(array('select'=>"tb.id, tb.photo, price.price, tb.url, price.discount, tb_lang.name, tb_lang.body, tb3.catalog_id, GROUP_CONCAT(DISTINCT tb3.catalog_id ORDER BY tb3.catalog_id ASC SEPARATOR ', ') as cat_id",
	...
	public function set_meta_data($data, $topic)///Мета-данные страницы
	{
        ...
        elseif(count($data)!=0)
        {
			...
			if($topic=='catalog')
			{
				...
				$catalog = $this->find(array('where'=>"`type`='4' AND active='1' AND title!=''"));
				if($catalog)
				{
			...
			elseif($topic=='product')
			{
				$product = $this->find(array('where'=>"`type`='5' AND active='1' AND title!=''"));
				if($product)
				{
			...
			else $data['title']=$prefix['title'].$data['name'].$suffix['title'];

			if(isset($data['keywords'])&&$data['keywords']!="")$data['keywords']=$data['keywords'];
			else $data['keywords']=$prefix['title'].$data['name'].$suffix['title'];

			if(isset($data['description'])&&$data['description']!="")$data['description']=$data['description'];
			else $data['description']=$prefix['title'].$data['name'].$suffix['title'];
