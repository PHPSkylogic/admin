...
class Product extends Model {
    ...
    public function add($open = false) {
        ...
        if (isset($_POST['active'], $_POST['url'], $_POST['name'], $_POST['title'], $_POST['keywords'], $_POST['description'], $_POST['body']) && $_POST['name'] != "") {
            if ($_POST['url'] == '')
                $url = String::translit($_POST['name']);
            else
                $url = String::translit($_POST['url']);

            $brend = "";
            if (isset($_POST['brend_id']))
                $brend = ",brend_id='{$_POST['brend_id']}'";

            if(!isset($_POST['one_price']))$_POST['one_price']=0;
            ...
            foreach ($languages as $lang) {
                ...
                $param = array($_POST['name'], $_POST['body'], $_POST['body2'], $_POST['body_m'], $insert_id);
                $this->db->query("INSERT INTO `$tb` SET `name`=?, `body`=?, `body2`=?, `body_m`=?, `product_id`=?", $param);
            }
                ...
                for ($i = 0; $i <= $count; $i++) {
                    $this->db->query("INSERT INTO `price` 
										SET code=?,
											...
											discount=?,
											price_type_id=?,
											product_id=?", array($_POST['price_code'][$i],
                        $_POST['price_price'][$i],
                        $_POST['purch_price'][$i],
                        $_POST['price_margin'][$i],
                        $_POST['price_unit'][$i],
                        $_POST['price_stock'][$i],
            ...
            if (isset($_POST['current_photo']) && file_exists($_POST['current_photo'])) {
                ...
                if (file_exists($photo_m))
                    copy($photo_m, $dir['0'] . $insert_id . "_m." . $ext);
            ...
            if (isset($_POST['save_id']) && is_array($_POST['save_id'])) {
                if (isset($_POST['save_id'], $_POST['code'], $_POST['price'])) {
                    ...
                    for ($i = 0; $i <= $count; $i++) {
                        ...
                        $this->db->query("UPDATE `" . $this->table . "` tb SET tb.`code`=?, tb.`sort`=? WHERE tb.id=?", array($_POST['code'][$i], $_POST['sort'][$i], $_POST['save_id'][$i]));
                        $this->db->query("UPDATE `price` SET `price`=? WHERE product_id=? AND price_type_id='1'", array($_POST['price'][$i], $_POST['save_id'][$i]));
                    }
                ...
                if (isset($_POST['active'], $_POST['url'], $_POST['id'], $_POST['name'], $_POST['title'], $_POST['keywords'], $_POST['description'], $_POST['body'])) {
                    ...
                    if ($row) {
                        $this->db->query("DELETE params_product FROM params_product 
										  ...
										  ON pps.params_product_id=params_product.id
                    ...
                    $res = $this->getColors($_POST['id']);
                    $colors = array();
                    foreach ($res as $row_c) {
                        $colors[] = array('name' => $row_c['name'], 'rgb' => $row_c['rgb'], 'photo' => $row_c['photo']);
                    }
                    ...
                    if(!isset($_POST['one_price']))$_POST['one_price']=0;
                    $param = array($_POST['code'], $_POST['unit'], $_POST['stock'], $colors, $_POST['name'], $_POST['body_m'], $_POST['body'], $_POST['body2'], $_POST['active'], $_POST['provider'], $_POST['provider_code'], $_POST['one_price'], $_POST['id']);
                    ...
                    if (isset($_POST['save_price_id'])) {
                        ...
                        for ($i = 0; $i <= $count; $i++) {
                            ...
                            if ($_POST['price_photo_id'][$i] != '')
                                $where = ",product_photo_id='{$_POST['price_photo_id'][$i]}'";
                            if ($_POST['price_texture_id'][$i] != '' && $_POST['price_texture_id'][$i] != 0)
                                $where.=",texture_photo_id='{$_POST['price_texture_id'][$i]}'";
                            else
                                $where.=",texture_photo_id='0'";


                            if($_POST['price_margin'][$i]>0&&$_POST['purch_price'][$i]>0)
                            {
                                $sum = Numeric::discount($_POST['price_margin'][$i], $_POST['purch_price'][$i]);
                                $_POST['price_price'][$i] = ($_POST['purch_price'][$i] - $sum)+$_POST['purch_price'][$i];
                            }
                            $this->db->query("UPDATE `price` 
												SET code=?,
													...
													price_type_id=?
													$where
												WHERE id=?", array($_POST['price_code'][$i],
                                $_POST['price_price'][$i],
                                $_POST['purch_price'][$i],
                                $_POST['price_margin'][$i],
                                $_POST['price_unit'][$i],
                    ...
                    if (isset($_POST['save_photo_id'])) {
                        ...
                        for ($i = 0; $i <= $count; $i++) {
                            if (isset($_POST['texture' . $_POST['save_photo_id'][$i]]))
                                $texture = 1;
                            else
                                $texture = 0;

                            $this->db->query("UPDATE `product_photo` SET texture='$texture' WHERE id=?", array($_POST['save_photo_id'][$i]));
    ...
    public function listView() {
        ...
        if (!isset($_SESSION['order']))
            $_SESSION['order'] = 'tb.sort';

        if (isset($this->params['page']) && $this->params['page'] > 0)
            $_SESSION['page_product'] = $this->params['page'];


        if (isset($this->params['order']) && $this->params['order'] == 'name')
            $_SESSION['order'] = 'tb_lang.name';
        elseif (isset($this->params['order']) && $this->params['order'] == 'catalog')
            $_SESSION['order'] = 'tb_cat_lang.name';
        elseif (isset($this->params['order']) && $this->params['order'] == 'id')
            $_SESSION['order'] = 'tb.id';
        elseif (isset($this->params['order']) && $this->params['order'] == 'price')
            $_SESSION['order'] = 'tb_price.price';
        elseif (isset($this->params['order']) && $this->params['order'] == 'sort')
            $_SESSION['order'] = 'tb.sort';
        elseif (isset($this->params['order']) && $this->params['order'] == 'active')
            $_SESSION['order'] = 'tb.active';

        if (isset($this->params['by']) && $this->params['by'] == 'asc')
            $_SESSION['by'] = 'desc';
        elseif (isset($this->params['by']) && $this->params['by'] == 'desc')
            $_SESSION['by'] = 'asc';

        if (isset($this->params['order']))
            $_SESSION['search_admin']['sort'] = $_SESSION['order'] . ' ' . $_SESSION['by'] . ', tb.id DESC';
        /////////////////sort end

        if ($_SESSION['search_admin']['word'] != "")
            $where = "AND (tb_lang.name LIKE '%{$_SESSION['search_admin']['word']}%' or 
        ...
        if ($_SESSION['search_admin']['price_from'] != "" && $_SESSION['search_admin']['price_to'] == "")
            $where.="AND (tb_price.price >='{$_SESSION['search_admin']['price_from']}')";
        elseif ($_SESSION['search_admin']['price_from'] == "" && $_SESSION['search_admin']['price_to'] != "")
            $where.="AND (tb_price.price <='{$_SESSION['search_admin']['price_to']}')";
        elseif ($_SESSION['search_admin']['price_from'] != "" && $_SESSION['search_admin']['price_to'] != "")
            $where.="AND (tb_price.price <='{$_SESSION['search_admin']['price_to']}' AND tb_price.price >='{$_SESSION['search_admin']['price_from']}')";
        ...
        if (!isset($_SESSION['search_admin']['sort']))
            $_SESSION['search_admin']['sort'] = "tb.sort DESC, tb.id DESC";
    ...
    public function save_pricetype() {
        ...
        if (isset($this->registry['access']))
            ...
            if (isset($_POST['name'], $_POST['price_id'])) {
                ...
                for ($i = 0; $i <= $count; $i++) {
                    $default = 0;
                    if ($_POST['default'] == $_POST['price_id'][$i])
                        $default = 1;

                    $param = array($_POST['name'][$i], $default, $_POST['price_id'][$i]);
    ...
    public function default_price_type($type = 'user')
    {
        ...
        {
            $row = $this->db->row("SELECT price_type_id AS id
                                   FROM `users`

                                   LEFT JOIN user_status
                                   ON user_status.id=users.status_id

                                   WHERE users.`id`=?", array($_SESSION['user_id']));
        }
        else{
    ...
    public function getColors($id) {
        return $this->db->rows("SELECT tb.*, tb2.name, GROUP_CONCAT(DISTINCT pp2.params_id SEPARATOR ',') as params_id, product_photo.photo
                                FROM `params` color
								...
								LEFT JOIN `params_product` pp2
                                ON color.id = pp2.params_id
								
								LEFT JOIN price
								ON price.id=pps.price_id
								
								LEFT JOIN product_photo
								ON product_photo.id=price.texture_photo_id

                                WHERE color.type='color' AND pp.product_id='$id' AND pps.price_id!=''
								GROUP BY tb.id
                            ");
    }
    ...
    public function getSizes($id) {
        return $this->db->rows("SELECT tb.id, tb2.name
                                ...
                                WHERE pp.product_id='$id' AND size.type='size' AND pps.price_id!='' AND price.price_type_id='{$_SESSION['price_type_id']}'
    ...
    function addPrice($id, $rel = '') {
        if ($rel != 'add') {
            ...
            $param = array($row['code'], $row['unit'], $row['stock'], $id, $this->default_price_type());
            $this->db->query("INSERT INTO price SET code=?, unit=?, stock=?, product_id=?, price_type_id=?", $param);
    ...
    function get_param($param_id, $product_id) {
        ...
        $res = $this->db->rows("SELECT ru.name, ru.params_id as id
								...
								WHERE p.id!='$param_id' AND price.price_type_id='{$_SESSION['price_type_id']}' $where
								
								GROUP BY ru.params_id
								ORDER BY price.product_photo_id DESC, ru.name ASC", array($product_id));
        //var_info($res);
        return $res;
    }
    ...
    function get_remains($param_id, $product_id) { 
        ...
        $row = $this->db->row("SELECT price.id, 
                                  ...
                                  ", array($product_id)); //var_info($row);
        //var_info($row);
        if ($row['stock'] > 0)
            $stock = '<div style="font-size:16px; color:#0a8527; margin-top:5px;" id="available">Есть в наличии</div>';
        else
            $stock = '<div style="font-size:16px; color:#aaa; margin-top:5px;">Нет в наличии</div>';
        ...
        else
        {
            if(isset($_POST['name'], $_POST['id']))
            {
                ...
                for($i=0; $i<=$count; $i++)
                {
                    ...
                    $param = array($_POST['name'][$i], $_POST['price'][$i], $default, $_POST['id'][$i]);
                    $this->db->query("UPDATE `providers` SET `name`=?, price=?, `default`=? WHERE id=?", $param);
                }
                $message .= messageAdmin('Данные успешно сохранены');
            }
