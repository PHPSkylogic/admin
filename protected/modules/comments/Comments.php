...
class Comments extends Model
{
    ...
    public function save()
    {
        ...
        else
        {
            ...
            {
                $param = array($_POST['name'], $_POST['email'], $_POST['phone'], $_POST['name2'], $_POST['text'], $_POST['text2'], $_POST['active'],$_SESSION['admin']['id'], $_POST['id']);
                $this->db->query("UPDATE `".$this->table."` SET `author`=?, `email`=?, `phone`=?, `author2`=?, `text`=?, `text2`=?, `active`=?, moderator_id=? WHERE `id`=?", $param);
	...
	public function addcomment()
    {
		if(isset($_POST['name'], $_POST['message'], $_POST['id'], $_POST['type'], $_POST['photo'])&&$_POST['name']!=""&&$_POST['message']!="")
		{
			...
			$query = "INSERT INTO `comments` SET `author`=?, `email`=?, `phone`=?, `text`=?, `content_id`=?, `type`=?, `date`=?, `session_id`=?, `language`=?, `photo`=? $where";
			$id = $this->db->insert_id($query, array(strip_tags($_POST['name']), strip_tags($_POST['email']), strip_tags($_POST['phone']), strip_tags($_POST['message']), $_POST['id'], $_POST['type'], $date, session_id(), $this->registry['key_lang'], $_POST['photo']));
			...
			if($_POST['image']!='')
			{
				$_POST['type_upload']=4;
				$_POST['image_id']=$id;
				$_POST['url']=$_POST['image'];
				$_POST['path']='files/comments/';
				$_POST['width']=435;
				$_POST['height']=326;
				$_POST['width2']='';
				$_POST['height2']='';
				$_POST['comments']='';
				include"protected/controllers/AjaxAdminController.php";
				$ajax=new AjaxAdminController($this->registry, $this->params);
				$ajax->includephotoAction();
				rename('files/comments/'.$_POST['image_id'].'_s.jpg', 'files/comments/'.$_POST['image_id'].'.jpg');
			}
