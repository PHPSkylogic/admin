    ...
    public function add()
    {
        ...
        if(isset($_POST['name'], $_POST['body'])&&$_POST['name']!="")
        {
            ...
            foreach($languages as $lang)
            {
                ...
                $this->db->query("INSERT INTO `$tb` SET `name`=?, `body`=?, `infoblocks_id`=?", $param);
            }
            ...
            if(isset($_POST['save_id'])&&is_array($_POST['save_id']))
            {
                if(isset($_POST['save_id'], $_POST['name']))
                {
                    for($i=0; $i<=count($_POST['save_id']) - 1; $i++)
                    {
                        ...
                        $this->db->query("UPDATE `".$this->registry['key_lang_admin']."_".$this->table."` SET `name`=? WHERE infoblocks_id=?", $param);
                    }
                ...
                if(isset($_POST['id'], $_POST['name'], $_POST['body']))
                {
                    ...
                    $this->db->query("UPDATE `".$this->registry['key_lang_admin']."_".$this->table."` SET `name`=?, `body`=? WHERE `infoblocks_id`=?", $param);
                    $message .= messageAdmin('Данные успешно сохранены');
                }
                else $message .= messageAdmin('При сохранение произошли ошибки', 'error');
            }
