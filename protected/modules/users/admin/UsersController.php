...
class UsersController extends BaseController{
	...
	public function editAction()
	{
		...
		$vars['status'] = $this->db->rows("SELECT * FROM user_status ORDER BY id ASC");
		$vars['sum'] = $this->db->row("SELECT SUM(tb2.`sum`) as total, COUNT(*) as cnt
		                               FROM `orders`

		                               LEFT JOIN orders_product tb2
		                               ON tb2.orders_id=orders.id

		                               WHERE orders.user_id=? OR orders.email=?
		                               ORDER BY orders.id
		                               ", array($vars['edit']['id'], $vars['edit']['email']));
    ...
    public function exportAction()
    {
        if(isset($_POST['download'])) {
            ...
            $dataSet = $this->users->find(array('select'=> $selectedColumns, 'type'=>'rows'));
