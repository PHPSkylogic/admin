...
class ConfigController extends BaseController{
	...
	public function indexAction()
	{
		...
		if($_SESSION['admin']['type']==1)
		{
			...
			$vars['menu'] = $this->db->rows("SELECT m.*, mp.*, COUNT(DISTINCT config.id) AS cnt  
											 FROM modules m
					...
					if(isset($_POST['save_id'], $_POST['comment']))
					{
						for($i=0; $i<=count($_POST['save_id']) - 1; $i++)
						{
							...
							if($this->model->check_for_update($id, $this->tb, $_SESSION['admin']['type']))
							{
								...
								//echo $value.'<br />';
								$param = array($value, $_POST['comment'][$i], $_POST['save_id'][$i]);
								$this->db->query("UPDATE `".$this->tb."` tb
	...
	public function listView($where='')
	{
		...
		$vars['list'] = $this->db->rows("SELECT tb.*
										 ...
										 GROUP BY tb.id
										 ORDER BY tb.`id` DESC, tb.modules_id ASC");
