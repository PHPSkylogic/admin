...
class TranslateController extends BaseController{
	...
	public function indexAction()
	{
		...
		if($_SESSION['admin']['type']==1)
		{
			...
			$vars['menu'] = $this->db->rows("SELECT m.*, mp.*, COUNT(DISTINCT translate.id) as cnt 
											 FROM modules m
			...
			if(isset($_POST['save_id'])&&is_array($_POST['save_id']))
			{
				if($_SESSION['admin']['type']==1)
				{
					if(isset($_POST['save_id'], $_POST['key'], $_POST['value'], $_POST['comment']))
					{
						...
						{
							if(isset($_POST['translate_to'])&&$_SESSION['key_lang_admin']!='ru')
							{
								$row = $this->db->row("SELECT `value` FROM ru_translate WHERE translate_id='{$_POST['save_id'][$i]}'");
								if($value = $translate->yandexTranslate('ru', $_SESSION['key_lang_admin'], $row['value']))
								{
									//echo $value.'==='.$this->key_translation;
									$sql = "UPDATE ".$this->tb_lang." SET `value`='$value' WHERE `translate_id`='{$_POST['save_id'][$i]}'";//echo $sql;
									$this->db->query($sql);
								}
							}
							else{
	...
	function listView($where='')
	{
		...
		$vars['list'] = $this->db->rows("SELECT tb.*, tb2.*
										 ...
										 $where		
										 ORDER BY tb.modules_id ASC, tb.`id` DESC");
