...
class AjaxAdminController extends BaseController{
	...
	function supportsendAction()
    {
        ...
        $body = "<table>
            <tr>
				<td>Адрес сайта: <br/></td><td>".$_SERVER['HTTP_HOST']." </td>
			</tr>
            <tr>
        ...
        Mail::send($this->settings['sitename'],
            ...
            "utf-8",
            "utf-8",
            "Новое сообщение в тех.поддержку на сайте ".$this->settings['sitename']." (".$subject.")",
            $body
        );
	...
	function includephotoAction()
	{
		...
		if($_POST['type_upload']!=1&&!isset($_POST['comments']))
		{
			$arr=json_decode($arr);
	...
	function type_upload($type_upload, $file, $path, $width, $height, $width2, $height2)
	{
		...
		elseif($type_upload==4)
		{
			$gab=getimagesize($file);
			if($gab[0]>$width)
			{
				$height = ($gab[1]/($gab[0]/$width));
				Images::resizeImage($file, $path.".jpg", $path."_s.jpg", $width, $height);
			}
			else{
				copy($file, $path."_s.jpg");	
			}
			unlink($file);
		}
		$return['ext'] = $ext;
