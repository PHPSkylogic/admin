...
class Numeric{
	...
	static function viewPrice($price, $discount=0)
	{
		...
		{
			$return['base_price'] = $price;var_info($_SESSION['currency']);
	...
	{
		if($currency=='')$currency=$_SESSION['currency'][1];
		if(!isset($_SESSION['rounding']))$_SESSION['rounding']=2;
		if($currency['position']==1)$price = '<span>'.number_format($price, $_SESSION['rounding'], ',', ' ').' </span> '.'<font>'.$currency['icon'].'</font>';
		else $price = '<font>'.$currency['icon'].'</font>'.number_format($price, $_SESSION['rounding'], ',', ' ');
		return $price;
	}
